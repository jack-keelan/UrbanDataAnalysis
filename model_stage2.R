...
##############################################################CURRENT!!!!!!!!!!!!!!!!!!
#df.stage1 <- read.csv("data/stage1_result.csv")
df.stage1 <- read.csv("data/stage1_result_Final.csv")
#df.stage1 <- read.csv("data/stage1_result_testset.csv")
df.stage1$ans_1sec <- factor(df.stage1$ans_1sec)
df.stage1$ans_2sec <- factor(df.stage1$ans_2sec)
df.stage1$ans_3sec <- factor(df.stage1$ans_3sec)
df.stage1$ans_4sec <- factor(df.stage1$ans_4sec)
df.stage1$V1 <- factor(df.stage1$V1)

#df.stage1 <- read.csv("data/stage1_loc.csv")
...
train <- df.stage1
#test <- df.stage1   ##### SKIP TO PREDICT!!!!!!!!

# train.indices <- c(1:(nrow(df.stage1)/2) )
# train <- df.stage1[train.indices,]
# test <- df.stage1[-train.indices,]
...
test <- read.csv("data/stage1_result_testset.csv")
test$ans_1sec <- factor(test$ans_1sec)
test$ans_2sec <- factor(test$ans_2sec)
test$ans_3sec <- factor(test$ans_3sec)
test$ans_4sec <- factor(test$ans_4sec)

preds <- predict(rf.cv, test, type = "raw")

#preds <- rbind(c(1:1225))

df.stage2 <- cbind(test,preds)

write.table(df.stage2, file = "data/stage2_result_testset.csv", 
            row.names=FALSE, col.names = TRUE, sep=",", na="0")
# write.table(df.stage2, file = "data/stage2_result_Final.csv", 
#             row.names=FALSE, col.names = TRUE, sep=",", na="0")
#############################################################
...
df_result$vote <- round(rowSums(df_result)/4)
