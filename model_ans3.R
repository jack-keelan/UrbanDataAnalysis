...
data_path <- "data/testingData/56Ch_rolling_3sec/"
vec_loc <- as.numeric(vector() )
vec_ans <- as.integer(vector() )
vec_lng <- as.integer(vector() )

#data_path <- "data/trainingData/56Ch_rolling_2sec_125s/"
intrv <- 12  ## number of quarter seconds in the integration window
...
test_3sec <- as.data.frame(cbind(ans_3sec,loc_3sec,lng_3sec))
write.table(test_3sec, file = "data/test_3sec.csv", 
            row.names=FALSE, col.names = TRUE, sep=",", na="0")
