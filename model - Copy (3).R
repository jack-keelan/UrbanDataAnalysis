...
##############################################################CURRENT!!!!!!!!!!!!!!!!!!
df.mod <- read.csv("data/TNG_metric_selected.csv")
...
df.new <- df.mod[,c(1:18)]
df.new$pR <- rowSums(df.mod[,c("P1","P2","P3","P4")])/rowSums(df.mod[,c("B1","B2","B3","B4")])
df.new$uR <- rowSums(df.mod[,c("U1","U2")])/rowSums(df.mod[,c("B1","B2","B3","B4")])
df.new$tR <- rowSums(df.mod[,c("T1","T2")])/rowSums(df.mod[,c("B1","B2","B3","B4")])
df.new$iR <- rowSums(df.mod[,c("I1","I2","I3","I4")])/rowSums(df.mod[,c("B1","B2","B3","B4")])
df.new$cR <- rowSums(df.mod[,c("C1","C2")])/rowSums(df.mod[,c("B1","B2","B3","B4")])
df.new$comR <- rowSums(df.mod[,c("U1","U2","T1","T2")])/rowSums(df.mod[,c("B1","B2","B3","B4")])

df.new$SourceID <- factor(df.mod$SourceID)
df.new$runID <- factor(df.mod$runID)
...
train.indices <- c(1:(nrow(df.new)/2) )
train <- df.new[train.indices,]
test <- df.new[-train.indices,]
...
features <- colnames(df.new[1:(nbins+1)])
...
saveRDS(rf.best, file="RF_metric_selected_withR_50per.rds")
saveRDS(rf.cv, file="RF_metric_selected_withR_FULL_50per.rds")  
