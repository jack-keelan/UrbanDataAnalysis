...
# 512 Channel and 512
input_path <- c("data/trainingData/integrated_228Ch_5Hz/")
#input_path <- c("data/trainingData/integrated_512Ch_12Hz/")
...
#output_path <- c("data/trainingData/new_runs_512Ch_12Hz/")  ## the output files will be 42.666...67 seconds
output_path <- c("data/trainingData/new_runs_224Ch_5Hz/")  ## the output files will be 44.8 seconds (0-2785.71857...)
...
Hz <- 5; old_location <- 0; win_size <- 224
#Hz <- 12; old_location <- 0; win_size <- 512
st_time <- Sys.time()
for(i in 1:9800){
  ...
  event <- event[,1:win_size]  ## resizing to 224x224 to match 
...
# #  df_evt$abs_loc[i] <- answers$SourceID[i]
  ...
  if(answers$SourceID[i] == 0){
    ...
    max_location <- old_location + as.integer(win_size)
    min_location <- old_location - as.integer(win_size) + 1
    if(min_location < 1){     # account for their being short/long runs
      min_location <- 1
      shift <- (old_location - as.integer(win_size) )
    }
    if(max_location > dim(event)[1]){ max_location <- dim(event)[1] }
    new_evt_temp <- subset( event[min_location:max_location,] )  ## may be shorter than 2*win_size
  }
  
  range_avl <- as.integer( dim(new_evt_temp)[1] - (win_size) - limit)  ##
  new_start <- as.integer( round(sample(limit:range_avl,1, replace = FALSE)) )
  if(range_avl <= limit){new_start <- limit}  ## exception when run is too short - remember sample will default w/ integer
  ## new_start will be the true index start of the 'new_evt_temp'
  ## new_end should just be new_start+(30*Hz)
  new_end <- new_start + (win_size) - 1

  if(answers$SourceID[i] != 0){
    new_location <- (win_size) - new_start + 1  # new start is opposit to source location
...
write.table(new_answers, file = paste(output_path,"new_answers", ".csv",sep = ""),  ## simple sum
            row.names=FALSE, col.names = TRUE, sep=",", na="0")

plot(1:512,colSums(new_event[10:220,]))
