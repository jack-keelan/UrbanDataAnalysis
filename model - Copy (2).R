...
sum(stats_comp$n) - stats_comp$n[21]
####
...
#  preds_dummy <- predict(mod, dummy, type = "response")  # 1 second
  ...
  if(max(s$n) >= 1 ){             ##  4 for the 3 sec integr set to 7 for 128Ch
...
df.stage2 <- read.csv("data/stage2_result_Final.csv")
...
data_path <- "data/testingData/56Ch_rolling_4sec/"
vec_loc <- as.numeric(vector() )
vec_ans <- as.integer(vector() )
vec_lng <- as.integer(vector() )
...
for(i in 1:15923){
  ...
  preds_dummy <- predict(mod, dummy, type = "raw")   #current bkg rm'ed
  preds_dummy[1:30] <- 0
  ...
  if(max(s$n) >= 1 ){             ##  4 for the 3 sec integr set to 7 for 128Ch
    ans_dummy <- as.integer(d) - 1
    loc_dummy <- round(median( which(as.integer(preds_dummy) %in% c(as.integer(d))) ),2)
    vec_loc[(i)] <- (((intrv*loc_dummy) + (1-intrv)*trunc(loc_dummy) + (intrv-1) )/4)
  } else {
    ans_dummy <- 0
    loc_dummy <- 0.00
    vec_loc[(i)] <- loc_dummy
...
df.ans1sec <- read.csv("data/test_1sec.csv")
df.ans2sec <- read.csv("data/test_2sec.csv")
df.ans3sec <- read.csv("data/test_3sec.csv")
df.ans4sec <- read.csv("data/test_4sec.csv")
ans_1sec <- df.ans1sec$ans_1sec; ans_2sec <- df.ans2sec$ans_2sec
loc_1sec <- df.ans1sec$loc_1sec; loc_2sec <- df.ans2sec$loc_2sec
ans_3sec <- df.ans3sec$ans_3sec; ans_4sec <- df.ans4sec$ans_4sec
loc_3sec <- df.ans3sec$loc_3sec; loc_4sec <- df.ans4sec$loc_4sec

df_result_test <- as.data.frame(cbind(ans_1sec, ans_2sec, ans_3sec, ans_4sec) )
df_loc_test <- as.data.frame(cbind(loc_1sec, loc_2sec, loc_3sec, loc_4sec) )

write.table(df_result_test, file = "data/stage1_result_testset.csv", 
            row.names=FALSE, col.names = TRUE, sep=",", na="0")

df.stage2 <- read.csv("data/stage2_result_testset.csv")

pred_loc <- as.numeric(vector())
for(i in 1:dim(df.stage2)[1]){
  pred <- as.integer(df.stage2[i,5])
  ans <- as.integer(df.stage2[i,1:4])
  loc <- as.numeric(df_loc_test[i,1:4])
  df <- as.data.frame(cbind(ans,loc))
...
vec_ans <- df.stage2$preds
vec_loc <- pred_loc

# vec_ans <- df.stage2$ans_1sec
# vec_loc <- df_loc_test$loc_1sec

vec_loc <- as.numeric(format(round(pred_loc, 2), nsmall = 2))
...
prev_sub2 <- read.csv("results/sub2.csv", header=TRUE, sep=",")
prev_sub3 <- read.csv("results/sub3.csv", header=TRUE, sep=",")
#write.csv(submission,"results/sub2.csv", row.names = FALSE)
#write.csv(submission,"results/sub3.csv", row.names = FALSE)
write.csv(submission,"results/sub9.csv", row.names = FALSE)  #first ensamble 56ch diff integration windows
