...
binData_path <- c("data/trainingData/128Ch_1sec/")
...
# event <- event[-c(1,2,3)]   # ONLY for Emma's format!!
  ...
  ##  Temp
  ans; dim(event)
  plot(1:128,colSums(event[58,1:128]))
  plot(1:60,colSums(event[57:59,1:60]))
  
  abline(v=(100/2650)*128); abline(v=(400/2650)*128); abline(v=(550/2650)*128)  #Pu
  abline(v=(364/2650)*128); abline(v=(635/2650)*128)  #I131
  # abline(v=(((140+186)/2650)*128)) 
  # abline(v=(186/2650)*128); abline(v=(140/2650)*128) #HEU & Tc
  (ans)/dim(event)[1]*144
...
output_path <- c("data/trainingData/56Ch_rolling_2sec/")  ## rolling  FIX THIS folder
...
#output_path <- c("data/testingData/56Ch_rolling_4sec/")  ## rolling
...
##  This line for rolling sum
  evt_roll <- as.data.frame(rollapply(event[1:(dim(event)[1]),], 8, sum) )
#####
